<x-app-layout>
    ...
    <div class="py-8 bg-gray-50">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white block md:hidden overflow-hidden shadow-2xl rounded-xl p-6 border border-gray-100">
                ...
                <div class="space-y-4">
                    @forelse($orderItems as $item)
                        <div class="bg-white p-4 rounded-lg shadow-md hover:shadow-lg transition duration-200
                                    grid grid-cols-1 lg:grid-cols-10 gap-4 items-center border border-gray-100">

                            <div class="col-span-full lg:col-span-2 flex justify-between items-start border-b lg:border-none pb-2 lg:pb-0">
                                <div class="flex flex-col items-start gap-0">
                                    <span class="font-mono font-bold text-gray-900 text-sm">#{{ $item->order->id }}</span>
                                    <span class="text-xs text-gray-600 font-medium lg:hidden">{{ $item->created_at->format('d/m/y') }}</span>
                                </div>

                                <div class="text-sm text-gray-600 font-medium hidden lg:block lg:col-span-1 lg:order-first">
                                    {{ $item->created_at->format('d/m/y') }}
                                </div>

                                <div class="flex flex-col items-end lg:hidden">
                                    <span class="text-xs text-gray-500 uppercase font-semibold">Customer</span>
                                    <span class="font-bold text-gray-800 text-sm">{{ $item->order->user->name }}</span>
                                </div>
                            </div>

                            <div class="col-span-full lg:col-span-4 flex flex-col gap-2 pt-2 lg:pt-0">
                            ...
                            <div class="lg:col-span-1 text-center text-sm font-bold text-gray-700 hidden lg:block">
                                {{ $item->quantity }}
                            </div>

                            <div class="lg:col-span-1 text-center hidden lg:block">
                                <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full {{ $color }}">
                                    {{ ucfirst($status) }}
                                </span>
                            </div>

                            <div class="col-span-full text-xs text-gray-500 border-t pt-2 mt-2 lg:col-span-10 lg:border-t-0 lg:pt-0 lg:mt-0">
                            ...
                            <div class="col-span-full lg:col-span-2 text-right pt-2 lg:pt-0">
                                <form action="{{ route('seller.orders.update-status', $item->order->id) }}" method="POST" class="flex justify-end gap-2 lg:mt-0">
                                    @csrf @method('PATCH')
            ...
            <div class="bg-white hidden md:block overflow-hidden shadow-2xl rounded-xl p-6 border border-gray-100">
