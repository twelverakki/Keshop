<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-2xl rounded-xl border border-gray-100 p-6">
                                    ...
                                    <td class="p-4 text-right">
                                        <div x-data="{ open: false }" @click.outside="open = false" class="relative inline-block text-left">
                                            ...
                                            <div x-show="open"
                                                ...
                                                <div class="py-1">
                                                    <a href="{{ route('seller.products.edit', $product->id) }}" class="text-gray-700 hover:bg-gray-100 px-4 py-2 text-sm flex items-center gap-2">
                ...
                <div class="sm:hidden space-y-4">
                    @foreach($products as $product)
                    <div class="border border-gray-200 rounded-lg shadow-sm p-4 bg-white space-y-3">
                        ...
                        <div class="flex justify-between items-center border-b border-gray-100 pb-3">
                            <div class="flex items-center gap-3">
                                <img src="{{ $product->image_url }}"
                                    ...
                                    alt="{{ $product->name }}">
                                <span class="font-bold text-gray-800">{{ $product->short_name }}</span>
                            </div>

                            <div class="flex-shrink-0">
                                @php $isActiveColor = $product->is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-500'; @endphp
                                <span class="px-3 py-1 text-xs font-semibold rounded-full {{ $isActiveColor }}">
                                    {{ $product->is_active ? 'Active' : 'Draft' }}
                                </span>
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-y-2 text-sm">
                            ...
                            <span class="font-medium text-right">{{ $product->category->name }}</span>

                            <span class="text-gray-500">Price:</span>
                            <span class="font-bold text-gray-900 text-right">Rp {{ number_format($product->price, 0, ',', '.') }}</span>

                            <span class="text-gray-500">Stock:</span>
                            <span class="text-right">
                                @php $stockColor = $product->stock < 10 ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'; @endphp
                                <span class="px-3 py-1 text-xs font-semibold rounded-full {{ $stockColor }}">
                                    {{ $product->stock }}
                                </span>
                            </span>
                        ...
                        <div class="pt-3 border-t border-gray-100 flex justify-end gap-2">
                             <a href="{{ route('seller.products.edit', $product->id) }}" class="text-sm font-medium text-blue-600 hover:text-blue-800 transition">Edit</a>

                             <form action="{{ route('seller.products.destroy', $product->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus produk ini?')" class="block">
                                 @csrf @method('DELETE')
                                 <button type="submit" class="text-sm font-medium text-red-600 hover:text-red-800 transition">Delete</button>
                             </form>
                        </div>
